...
namespace Scripts
{
    ...
    public sealed class EnemyWaveSpawner : MonoBehaviour, IInitializable, IDisposable
    {
        ...
        // Same off-screen ring logic used previously (ported from your presenter). :contentReference[oaicite:1]{index=1}
        private static Vector3 GetOffScreenPosition(Camera camera, float padding, System.Random rng)
        {
            ...
            switch (side)
            {
                case 0: return new Vector3(left - padding, UnityEngine.Random.Range(bottom, top), 0f);
                case 1: return new Vector3(right + padding, UnityEngine.Random.Range(bottom, top), 0f);
                case 2: return new Vector3(UnityEngine.Random.Range(left, right), bottom - padding, 0f);
                default: return new Vector3(UnityEngine.Random.Range(left, right), top + padding, 0f);
            }
        }
    }
}
