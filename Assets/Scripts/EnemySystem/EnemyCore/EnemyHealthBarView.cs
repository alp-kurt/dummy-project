...
namespace Scripts
{
    public sealed class EnemyHealthBarView : MonoBehaviour
    {
        ...
        private void Bind(IEnemyModel model)
        {
            ...
            // Update health bars every spawn
            Observable.EveryUpdate().Where(__ => _view.Model != _model && _view.Model != null)
                .Take(1).Subscribe(__ => { _cd.Clear(); Bind(_view.Model); }).AddTo(_cd);
        }
